...
const ADVENTURE_CATEGORIES = [
  { name: 'Urban Exploration', icon: '🏙️', description: 'Discovering hidden gems in city centers', color: 'from-blue-500 to-blue-300' },
  { name: 'Nature Escapes', icon: '🌲', description: 'Connecting with the natural world', color: 'from-green-500 to-green-300' },
  { name: 'Cultural Immersion', icon: '🏛️', description: 'Experiencing local traditions and history', color: 'from-amber-500 to-amber-300' },
  { name: 'Adventure Sports', icon: '🧗‍♂️', description: 'Pushing limits with thrilling activities', color: 'from-red-500 to-red-300' }
];

// Travel stats with icons and descriptions
const TRAVEL_STATS = [
  { icon: '🌏', number: '15+', label: 'Countries', description: 'Explored across 4 continents', color: 'from-blue-500 to-blue-300' },
  { icon: '🎒', number: '100+', label: 'Adventures', description: 'From mountains to beaches', color: 'from-purple-500 to-purple-300' },
  { icon: '📸', number: '1000+', label: 'Moments', description: 'Captured and shared', color: 'from-red-500 to-red-300' },
  { icon: '🍜', number: '50+', label: 'Cuisines', description: 'Tasted and experienced', color: 'from-green-500 to-green-300' }
];
...
const DaxTheTravelerPage = () => {
  ...
  // -------- PERFORMANCE FIX --------
  // Use requestAnimationFrame mouse tracking (no React state = no re-renders)
  ...
  const handleYouTubeDataLoaded = useCallback((helpers) => {
    ...
    // Keep your existing brand key to avoid behavior changes
  ...
  useEffect(() => {
    if (loading) return;
    let mounted = true;
    const id = setInterval(() => mounted && setActiveDestination((prev) => (prev + 1) % TRAVEL_DESTINATIONS.length), 7000);
  ...
  useEffect(() => {
    let mounted = true;
    const id = setInterval(() => mounted && setActiveCategory((prev) => (prev + 1) % ADVENTURE_CATEGORIES.length), 5000);
    return () => { mounted = false; clearInterval(id); };
  }, []);
  ...
  // Stats (memoized to prevent trivial re-renders)
  const stats = useMemo(() => ([
    {
...
const VideoCard = React.memo(function VideoCard({ video, isShort = false, onClick }) {
    ...
    >
      <div className="relative">
        <img
          ...
          loading="lazy"
          decoding="async"
          onError={(e) => { e.target.src = 'https://via.placeholder.com/400x225?text=Dax+The+Traveler'; }}
        />
...
VideoCard.displayName = 'VideoCard';
            ...
            <div className="text-center">
              <div className="mb-6">
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  onError={(e) => { e.target.src = 'https://via.placeholder.com/160x80?text=Dax+The+Traveler'; }}
                />
              </div>
  ...
  const AdventureCategoryCard = ({ category, index, isActive }) => (
    <motion.div
      ...
      animate={{ opacity: isActive ? 1 : 0.7, scale: isActive ? 1 : 0.95, y: isActive ? 0 : 10 }}
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="mb-1 font-bold text-white">{category.name}</div>
  ...
  const TravelStatCard = ({ stat, index }) => {
    ...
    return (
      <motion.div
        ...
        whileHover={{ scale: 1.05, rotate: -1, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04)' }}
        onHoverStart={() => setIsHovered(true)}
        onHoverEnd={() => setIsHovered(false)}
      ...
      >
        ...
        <div className="p-5">
          ...
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">{stat.number}</span>
  ...
  const AdventuresCollage = () => (
    ...
    >
      <img
        ...
        loading="lazy"
        decoding="async"
        onError={(e) => { e.target.src = 'https://via.placeholder.com/800x600?text=Adventures'; }}
      />
      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "sameAs": [videoData.channelInfo?.url],
      ...
      <div className="bg-gray-900">
        ...
        <section ref={heroRef} className="relative h-screen overflow-hidden">
          ...
          >
              ...
              <div className="max-w-7xl mx-auto px-4 py-6 flex justify-between items-center">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="flex items-center gap-3">
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    onError={(e) => { e.target.src = 'https://via.placeholder.com/160x40?text=Dax'; }}
                  />
                </motion.div>

                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="flex items-center gap-6">
                  <button onClick={() => scrollToSection(aboutRef)} className="text-gray-300 hover:text-white transition-colors">About</button>
                  <button onClick={() => scrollToSection(videosRef)} className="text-gray-300 hover:text-white transition-colors">Videos</button>
                  <button onClick={() => scrollToSection(galleryRef)} className="text-gray-300 hover:text-white transition-colors">Gallery</button>
                  <button onClick={() => scrollToSection(connectRef)} className="text-gray-300 hover:text-white transition-colors">Connect</button>
              ...
              <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                ...
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center lg:text-left">
                  <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight">
                    ...
                    <span className="block bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Traveler</span>
                  </h1>
                  ...
                  <div className="flex flex-col lg:flex-row items-center gap-8 mb-8">
                    ...
                    <div>
                      <p className="text-xl text-gray-300 mb-4 max-w-xl">Real adventures, authentic experiences, and practical travel wisdom from someone who's been there.</p>

                      <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => scrollToSection(videosRef)} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-full font-semibold text-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-300 shadow-lg flex items-center gap-2">
                          <PlayIcon className="w-5 h-5" />
                          Watch Adventures
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowNewsletter(true)} className="bg-white/10 backdrop-blur-sm border-2 border-white/30 text-white px-8 py-4 rounded-full font-semibold text-lg hover:bg-white/20 transition-all duration-300">
                          Join Newsletter
                        </motion.button>
                      </div>
                    </div>
                  ...
                  {videoData.channelInfo && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.8 }} className="mt-4 text-gray-300 flex items-center justify-center lg:justify-start gap-2">
                      <p className="text-sm">Follow me on YouTube:</p>
                      <motion.button whileHover={{ scale: 1.05, x: 3 }} onClick={() => openExternalLink(videoData.channelInfo.url)} className="text-blue-400 hover:text-blue-300 transition-colors underline flex items-center gap-1">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {ADVENTURE_CATEGORIES.map((category, index) => (
                      <AdventureCategoryCard key={category.name} category={category} index={index} isActive={index === activeCategory} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            <AnimatePresence>
              {showScrollHint && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.5, repeat: Infinity, repeatType: 'reverse' }} className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white text-center">
          ...
          <section className="py-20 px-4 bg-gradient-to-b from-gray-900 to-gray-800">
            <div className="max-w-7xl mx-auto">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-12">
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Featured Adventure</h2>
                <div className="w-24 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto"></div>
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} className="bg-gray-800/30 backdrop-blur-sm rounded-2xl overflow-hidden border border-gray-700/50">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
                  ...
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-2xl font-bold text-white mb-4">{videoData.featured.title}</h3>
                      <p className="text-gray-300 leading-relaxed">{videoData.featured.description?.substring(0, 300)}...</p>
                    </div>

                    <div className="grid grid-cols-3 gap-4">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-400">{formatNumber(videoData.featured.viewCount)}</div>
                        <div className="text-sm text-gray-400">Views</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-red-400">{formatNumber(videoData.featured.likeCount)}</div>
                        <div className="text-sm text-gray-400">Likes</div>
                      </div>
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-400">{videoData.featured.durationFormatted || '0:00'}</div>
                        <div className="text-sm text-gray-400">Duration</div>
                      </div>
                    </div>

                    <div className="flex gap-4">
                      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openExternalLink(videoData.featured.watchUrl || `https://youtube.com/watch?v=${videoData.featured.videoId}`)} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2">
                        <PlayIcon className="w-5 h-5" />
                        Watch on YouTube
                      </motion.button>
        ...
        {videoData.shorts && videoData.shorts.length > 0 && (
          <section ref={videosRef} className="py-20 px-4 bg-gradient-to-b from-gray-800 to-gray-900">
            <div className="max-w-7xl mx-auto">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-12">
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Quick Adventures</h2>
                <div className="w-24 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto"></div>
              </motion.div>

              <div className="relative">
                <div ref={shortsContainerRef} className="flex gap-6 overflow-x-auto scrollbar-hide pb-4 justify-center" style={{ scrollSnapType: 'x mandatory' }}>
                  {videoData.shorts.map((video) => (
                    <div key={video.videoId} className="flex-shrink-0" style={{ scrollSnapAlign: 'start' }}>
                      <YouTubeEmbed video={video} isShort={true} className="w-64 h-[456px]" />
                ...
                {videoData.shorts.length > 1 && (
                  <>
                    <motion.button whileHover={{ scale: 1.1, x: -3 }} whileTap={{ scale: 0.9 }} onClick={() => { shortsContainerRef.current?.scrollBy({ left: -280, behavior: 'smooth' }); }} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-colors">
                      <ChevronLeftIcon className="w-6 h-6" />
                    </motion.button>
                    <motion.button whileHover={{ scale: 1.1, x: 3 }} whileTap={{ scale: 0.9 }} onClick={() => { shortsContainerRef.current?.scrollBy({ left: 280, behavior: 'smooth' }); }} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-colors">
        ...
        {videoData.longform && videoData.longform.length > 0 && (
          <section className="py-20 px-4 bg-gradient-to-b from-gray-900 to-gray-800">
            <div className="max-w-7xl mx-auto">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-12">
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Travel Adventures</h2>
                <div className="w-24 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto"></div>
              </motion.div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {videoData.longform.slice(0, 6).map((video) => (
                  <VideoCard key={video.videoId} video={video} onClick={() => openExternalLink(video.watchUrl || `https://youtube.com/watch?v=${video.videoId}`)} />
                ))}
              </div>

              {videoData.longform.length > 6 && (
                <div className="text-center mt-8">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openExternalLink(videoData.channelInfo?.url ? `${videoData.channelInfo.url}/videos` : '#')} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg px-6 py-3 text-white hover:bg-white/20 transition-all duration-300">
                    View All Adventures
                  </motion.button>
                </div>
              )}
        ...
        <section ref={aboutRef} className="py-20 px-4 bg-gradient-to-b from-gray-800 to-gray-900">
          <div className="max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-16">
              <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">About Dax</h2>
              <div className="w-24 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto"></div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="space-y-6">
                <h3 className="text-2xl font-bold text-white mb-4">Solo Traveler & Authentic Storyteller</h3>
                ...
                <div className="flex gap-4 mt-8">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openExternalLink(videoData.channelInfo?.url || '#')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors flex items-center gap-2">
                    <span>📺</span>
                    Subscribe on YouTube
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => scrollToSection(connectRef)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors">
                    Let's Connect
                  </motion.button>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} className="space-y-6">
                <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50">
                  ...
                  <ul className="space-y-3 text-gray-300">
                    <li className="flex items-start gap-2"><span className="text-blue-400 mt-1">•</span><span>Authentic experiences over tourist traps</span></li>
                    <li className="flex items-start gap-2"><span className="text-blue-400 mt-1">•</span><span>Budget-conscious but not cheap</span></li>
                    <li className="flex items-start gap-2"><span className="text-blue-400 mt-1">•</span><span>Solo travel builds confidence</span></li>
                    <li className="flex items-start gap-2"><span className="text-blue-400 mt-1">•</span><span>Every mistake is a story</span></li>
                  </ul>
                </div>
                ...
                <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50">
                  ...
                  <ul className="space-y-3 text-gray-300">
                    <li className="flex items-start gap-2"><span className="text-purple-400 mt-1">•</span><span>Real-time travel vlogs</span></li>
                    <li className="flex items-start gap-2"><span className="text-purple-400 mt-1">•</span><span>Practical travel tips</span></li>
                    <li className="flex items-start gap-2"><span className="text-purple-400 mt-1">•</span><span>Budget breakdowns</span></li>
                    <li className="flex items-start gap-2"><span className="text-purple-400 mt-1">•</span><span>Cultural insights</span></li>
                  </ul>
                </div>
              </motion.div>
            </div>
        ...
        <section ref={galleryRef} className="py-20 px-4 bg-gradient-to-b from-gray-900 to-gray-800">
          <div className="max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-16">
              <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Travel Gallery</h2>
              <div className="w-24 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto"></div>
            </motion.div>
          ...
          <div className="max-w-7xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} viewport={{ once: true }} whileHover={{ scale: 1.03, y: -5 }} className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
                  ...
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openExternalLink(videoData.channelInfo?.url || '#')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg transition-colors">
                    Subscribe
                  </motion.button>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} whileHover={{ scale: 1.03, y: -5 }} className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
                  <div className="text-4xl mb-4">📧</div>
                  <h3 className="text-xl font-bold text-white mb-2">Newsletter</h3>
                  <p className="text-gray-300 mb-4">Exclusive tips and updates</p>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowNewsletter(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition-colors">
                    Join Now
                  </motion.button>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} viewport={{ once: true }} whileHover={{ scale: 1.03, y: -5 }} className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
                  <div className="text-4xl mb-4">🤝</div>
                  <h3 className="text-xl font-bold text-white mb-2">Collaborate</h3>
                  <p className="text-gray-300 mb-4">Brand partnerships welcome</p>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => (window.location.href = 'mailto:hello@daxcollective.com')} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg transition-colors">
                    Get in Touch
                  </motion.button>
                </motion.div>
              </div>
